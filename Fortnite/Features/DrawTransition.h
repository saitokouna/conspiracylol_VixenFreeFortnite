...
void DrawTransition( UGameViewportClient* ViewPortClient, UCanvas* Canvas )
{
	...
	Variables::FieldOfView = FieldOfView * ( 180.f / M_PI );
	...
	if ( AcknowledgedPawn )
	{
		...
		if ( Settings::Environment::Pickup )
		{
			...
			for ( int i = 0; i < PickupArray.NumElements; i++ )
			{
				...
				if (Settings::Developer::WeaponPickupIconTest)
				{
					auto SmallPreviewImage = PickupItemDefinition->GetSmallPreviewImage( );

					if (SmallPreviewImage.Get())
					{
						UTexture* Texture = UObject::FindObjectSingle<UTexture*>(StaticClasses::KismetStringLibrary->Conv_NameToString
						(SmallPreviewImage.ObjectID.AssetPathName).c_str());

						if (Texture)
						{
							Canvas->K2_DrawTexture(Texture, FVector2D(ScreenLocation.X, ScreenLocation.Y - (FontSize + FontSize + 2.0)), 
								FVector2D(25.0, 25.0), FVector2D(), FVector2D(1.0, 1.0), FLinearColor(1.f, 1.f, 1.f, 1.f),
								EBlendMode::BLEND_Translucent, 0.f, FVector2D());
						}
					}
				}
	...
	for ( int32_t i = 0; i < PlayerList.NumElements; i++ )
	{
		...
		if ( PlayerCheck && Player != AcknowledgedPawn )
		{
						...
						for (int i = 0; i < PawnMaterials.NumElements; i++)
						{
							if (PawnMaterials.Valid(i))
							{
								if (Settings::Exploits::ChamsThroughWalls)
								{
									...
									{
										LocalMaterial->SetbDisableDepthTest(true);
									}
								}
								else
								{
									if (UMaterial* LocalMaterial = (UMaterial*)PawnMaterials[i]->GetMaterial())
									{
										LocalMaterial->SetbDisableDepthTest(false); 
									}
								}
							}
						}
			...
			if (Settings::Developer::bHasGObjectBeenValidated)
			{
				if (Settings::Exploits::Chams && Classes::Material)
				{
					if (!Material)
					{
						Material = ObjectArray->FindObject<UMaterial*>(StaticClasses::KismetStringLibrary->Conv_StringToName(L"RezIn_Master"), Classes::Material);
					}

					if (Material) {

						Material->SetBlendMode(EBlendMode::BLEND_Additive);
					...
					for (int32_t xi = 0; xi < SkeletalMeshes.NumElements; xi++)
					{
						if (!SkeletalMeshes.Valid(xi)) continue;

						USkeletalMeshComponent* SkeletalMesh = SkeletalMeshes[xi];
						if (!SkeletalMesh) continue;

						TArray<UMaterialInterface*> Materials = SkeletalMesh->GetMaterials();
						for (int32_t ri = 0; ri < Materials.NumElements; ri++)
						{
							if (!Materials.Valid(ri)) continue;

							//StoredElementIndexs[ ri ] = SkeletalMesh->GetMaterial( ri );

							UMaterialInstanceDynamic* DynamicMaterialInstance = SkeletalMesh->CreateDynamicMaterialInstance(ri, Material, FName());
							if (!DynamicMaterialInstance) continue;

							FLinearColor RenderColor = FLinearColor(0.190463f, 0.737205f, 1.f, 2.f);
							DynamicMaterialInstance->SetVectorParameterValue(StaticClasses::KismetStringLibrary->Conv_StringToName(L"WireFrameParameterHighlight"), RenderColor);
							DynamicMaterialInstance->SetVectorParameterValue(StaticClasses::KismetStringLibrary->Conv_StringToName(L"WireFrameFadeOffColor"), RenderColor);
							DynamicMaterialInstance->SetVectorParameterValue(StaticClasses::KismetStringLibrary->Conv_StringToName(L"Top Color"), RenderColor);
							DynamicMaterialInstance->SetVectorParameterValue(StaticClasses::KismetStringLibrary->Conv_StringToName(L"Bottom Color"), RenderColor);

							SkeletalMesh->SetMaterial(ri, DynamicMaterialInstance);
						}
					}

					ChamsWasEnabled = true;
				}
				else
				{
					if (ChamsWasEnabled)
					{
						...
						for (int32_t xi = 0; xi < SkeletalMeshes.NumElements; xi++)
						{
							...
							{
								if (!Materials.Valid(ri)) continue;
								if (!SkeletalMesh->GetMaterial(ri)) continue;

								//auto it = StoredElementIndexs.find( ri );
								//if ( it != StoredElementIndexs.end( ) )
								//	SkeletalMesh->SetMaterial( ri , it->second );

								SkeletalMesh->SetMaterial(ri, nullptr);
							}
						}

						ChamsWasEnabled = false;
					}
				}
			}
			...
			if (Settings::Player::Enable)
			{
				...
				if ( Settings::Player::Radar )
				{
					...
					{
						double RadarSize = ( 15.625 * Variables::ScreenCenter.X * double( ClientSettingsRecord->HUDScale( ) ) / 100.f ) * 2.0;

						FVector2D RadarPosition = FVector2D( Settings::Player::RadarX , Settings::Player::RadarY );

						Radar::Setup( RadarPosition , FVector2D( RadarSize , RadarSize ) );
						Radar::Add( Root , BoxColor , RadarPosition , FVector2D( RadarSize , RadarSize ) );
					}
				}
							...
							if (Mesh)
							{
								...
								if (Custom::K2_Project(AimbotPosition, &ScreenLocation))
								{
									...
									if ((Settings::Aimbot::VisibleOnly && IsVisble || !Settings::Aimbot::VisibleOnly) && AcknowledgedPawn)
									{
										...
										if (Settings::Aimbot::Triggerbot && World)
										{
											if ((Settings::Aimbot::TriggerbotConstant) || (Settings::Aimbot::TriggerbotLeftShift && PlayerController->IsInputKeyDown(Keys::LeftShift)))
											{
												static double LastTriggerBotTick = 0.0;
												double CurrentTime = StaticClasses::KismetSystemLibrary->GetGameTimeInSeconds(World);

												if ((CurrentTime - LastTriggerBotTick) >= Settings::Aimbot::TriggerbotDelay) // delay
												{
													const bool isAlreadyShooting = PlayerController->WasInputKeyJustPressed(Keys::LeftMouseButton);

													if (!isAlreadyShooting)
													{
														for (int i = 0; i < CurrentWeapon->GetRemainingAmmo(); i++)
														{
															if (CurrentTime > (LastTriggerBotTick - Settings::Aimbot::TriggerbotInterval))
															{
																WindowsAPI::mouse_event(MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0);
															}
														}
													}
													else
													{
														if (CurrentTime > (LastTriggerBotTick - Settings::Aimbot::TriggerbotDelay))
														{
															WindowsAPI::mouse_event(MOUSEEVENTF_LEFTUP, 0, 0, 0, 0);
														}
													}
												}
											}
	...
	if (Settings::Player::CrosshairMode)
	{
		...
		else if (Settings::Player::CrosshairMode == 5 && World && Canvas)
		{
			...
			{
				std::vector<int> p;
				p.push_back(static_cast<int>(a * sin(GRD_TO_BOG(RotationAngle + (i * 90)))));                                    // p[0]        p0_A.x
				p.push_back(static_cast<int>(a * cos(GRD_TO_BOG(RotationAngle + (i * 90)))));                                    // p[1]        p0_A.y
				p.push_back(static_cast<int>((a / cos(gamma)) * sin(GRD_TO_BOG(RotationAngle + (i * 90) + BOG_TO_GRD(gamma))))); // p[2]        p0_B.x
				p.push_back(static_cast<int>((a / cos(gamma)) * cos(GRD_TO_BOG(RotationAngle + (i * 90) + BOG_TO_GRD(gamma))))); // p[3]        p0_B.y
				...
				float hue = fmodf(StaticClasses::GameplayStatics->GetTimeSeconds(World), 0.50f);
				FLinearColor lineColor = Wrapper::HSVtoRGB(hue + i * 0.25f, 1.0f, 1.0f);;

				if (Settings::Misc::bGermanyCrosshairNiggerMode)
				{
					lineColor = FLinearColor(FromRGB(0, 0, 0));
				}
